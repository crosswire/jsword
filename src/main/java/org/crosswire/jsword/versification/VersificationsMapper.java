...
public class VersificationsMapper {
    ...
    public Key map(Verse v, Versification targetVersification) {
        ...
        if (mapper == null) {
            // we can't map to the KJV, so we're going to take a wild guess and return the equivalent verse
            // and assume that it maps directly on to the KJV, and thereby continue with the process
        ...
        //we're continuing, so we need to unmap from the KJV qualified key onto the new versification.
        VersificationToKJVMapper targetMapper = MAPPERS.get(targetVersification);
        if (targetMapper == null) {
            //failed to load, so we'll do our wild-guess again, and assume that the KJV keys map to the
            //target
            return guessKeyFromKjvVerses(targetVersification, kjvVerses);
        }

        //we can use the unmap method for that. Since we have a list of qualified keys, we do so for every qualified
        //key in the list - this means that parts would get transported as well.
     ...
     * @return the possible verses in the target versification, no guarantees made
     */
    private Key guessKeyFromKjvVerses(final Versification targetVersification, final List<QualifiedKey> kjvVerses) {
        ...
        return finalKeys;
            ...
            //we swallow the exception, as we've already alerted that we failed to load the missing resources.
    ...
    private Key getKeyFromQualifiedKeys(Versification versification, final List<QualifiedKey> kjvVerses) {
        ...
        for (QualifiedKey k : kjvVerses) {
            //we simply ignore everything else at this stage. The other bits and pieces are used while we're converting
