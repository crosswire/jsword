...
public class StartsParamWord implements ParamWord
{
    ...
    */
    ...
    {
        throw new SearchException("search_starts_word");
    }
    ...
    */
    public Passage getPassage(Engine engine) throws SearchException
    {
        if (!engine.iterator().hasNext())
            throw new SearchException("search_starts_blank");

        try
        {
            ParamWord param = (ParamWord) engine.iterator().next();
            String word = param.getWord(engine);
            String[] words = BookUtil.toStringArray(engine.getBible().getStartsWith(word));
            return BookUtil.getPassage(engine.getBible(), words);
        }
        catch (Exception ex)
        {
            throw new SearchException("search_starts_other", ex);
        }
    }
}
