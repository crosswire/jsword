...
public class BlurCommandWord implements CommandWord
{
    ...
    public void updatePassage(Engine engine, Passage ref) throws SearchException
    {
        if (!engine.iterator().hasNext())
            throw new SearchException("search_blur_blank");
        ...
        try
        {
            ref.blur(Integer.parseInt(word), PassageUtil.getBlurRestriction());
        }
        catch (NumberFormatException ex)
        {
            throw new SearchException("search_blur_format", ex, new Object[] { word });
        }
    }
}
